...
st.title('**YouTube Data Harvesting and Warehousing using SQL and Streamlit**', anchor = False)
...
if selected == "Home":
  ...
  with col1:
   ...
   with st.container(height=400):
     st.markdown("#### Retrieving the Youtube channels data from Google Console, creating and transforming the data into a SQL database as a Data Warehouse and querying the data to display in Streamlit.")
...
if selected == "Retrieval & Transformation":
  ...
  with tab1:
    ...
    if channel_id and st.button("Convert Data to MongoDB"):
...
###########################################################################################
  ...
  with tab2:
    st.header(":red[Select a channel to begin Transformation to MYSQL SERVER]", anchor = False)
    st.divider()
 ...
 # Convert Duration column values to minutes format   
    ...
    if st.button("Upload"):
      try:
        ...
        st.success("Channel Uploaded successfully to Mysql Server.....")
      except:
        st.error("Channel ID already uploaded into Mysql!!!") 
